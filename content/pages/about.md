---
title: "About Sakanosita IC Labs | smacon.dev"
template: about
profile_image: /media/profile/profile.jpg
description: Hi, I'm Moto, Web3 programmer and working at Edtech. Also I enjoy making smart contracts and building Dapps.
---

# Welcome to Sakanosita IC Labs! 🎉

Hey there! We're **Sakanosita IC Labs**, founded in 2021 by our fearless leader, Moto Sakanosita. We're a bunch of tech enthusiasts who love working on innovative projects and sharing our knowledge with the world. You can find our work at [https://github.com/sakanosita-ic-labs](https://github.com/sakanosita-ic-labs).

## What we do 🚀

At Sakanosita IC Labs, we're all about exploring the latest and greatest in technology. We work on a wide range of projects, from software development to hardware design, and everything in between. Our goal is to push the boundaries of what's possible and to inspire others to do the same.

## Get in touch 📬

Got questions or just want to chat? Feel free to reach out to us! You can find me on [Twitter](https://twitter.com/zk_moto).
