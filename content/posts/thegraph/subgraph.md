...
なるべく簡単な方法を紹介していますが、サブグラフを使うには npm や GraphQL や Solidity の最低限の理解が必要になります。
...
subgraph.yaml で以下のようにどのブロックからデータの取得を開始するかを指定します。
...
これを指定しないとコントラクトをデプロイする前のブロックのログを分析してしまうので無駄に時間がかかります。
